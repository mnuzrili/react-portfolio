...
const Navbar = () => {
  ...
  return (
    <nav className={`fixed w-full left-0 top-0 z-[999] ${sticky ? "bg-white/20  text-white-900" : "text-white"}`}>
        ...
        <div className={`${sticky ? "md:bg-white/0 bg-white" : "bg-white"} text-gray-900 md:block hidden px-7 py-2 font-medium rounded-bl-full`}>
          <ul className="flex items-center gap-1 py-2 text-lg">
            {menuLinks?.map((menu, i) => (
              <li key={i} className={`px-6 hover:text-pink-600 ${sticky ? "md:text-grey-900 text-white" : "text-grey-900"}`}>
